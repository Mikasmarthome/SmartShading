...
async def _async_setup_zone_entry(
    ...
    # Coordinator setup
    # ------------------------------------------------------------------
    # T21 Phase C3: absence_delay_min may defer to the System entry's global
    # default — same resolve_zone_*(zone_raw, system_raw) pattern as
    # resolve_zone_override_policy()/resolve_zone_dispatch_config() above.
    _effective_absence_delay_min = resolve_zone_absence_delay_min(entry.data, _system_raw)
    coordinator = SmartShadingCoordinator(
        ...
        absence_delay_min=_effective_absence_delay_min,
