...
Tier 1 currently contains STORM_SAFE, WIND_SAFE and RAIN_SAFE.
...
class ShadingState(Enum):
    ...
    Tier 1 Safety cluster (ranks 1–9):
      ...
      RAIN_SAFE  — opt-in (default on for awnings/screens, off for roller shutters);
                   retracts covers during rain; released after configurable dry cooldown.
      Ranks 4–9 are reserved for future Tier-1 states (e.g. Frost Protection once
...
STATE_PRIORITY: dict[ShadingState, int] = {
    # Tier 1 — Safety Guards (ranks 1–9 reserved; 4–9 available for future Tier-1)
    ShadingState.STORM_SAFE:      1,
    ShadingState.WIND_SAFE:       2,
    ShadingState.RAIN_SAFE:       3,
